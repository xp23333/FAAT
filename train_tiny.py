...
def parse_args():
    parser = argparse.ArgumentParser(description='PyTorch Tinyimagenet Adversarial Training')
    ...
    parser.add_argument('--dataset', default='tinyimagenet', choices=['tinyimagenet'],
...
if args.arch == 'preactresnet18':
    netG = PreActResNet18(num_classes=n_classes, stride=2).to(device)
elif args.arch == 'wideresnet34':
    netG = WideResNet(device=device, depth=34, num_classes=n_classes, stride=2).to(device)
else:
